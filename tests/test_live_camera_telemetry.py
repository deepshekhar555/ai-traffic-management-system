import importlib.util
import os


ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
MODULE_PATH = os.path.join(ROOT, 'backend', 'dashboard_app.py')


spec = importlib.util.spec_from_file_location('dashboard_app_mod', MODULE_PATH)
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)


def test_live_camera_telemetry_has_meaningful_fallback_data():
    response = module.app.test_client().get('/api/live-camera-telemetry')
    assert response.status_code == 200
    data = response.get_json()
    assert 'vehicle_count' in data
    assert 'person_count' in data
    assert 'total_count' in data
    assert data['vehicle_count'] >= 0
    assert data['person_count'] >= 0
    assert data['total_count'] >= 0
    assert data['total_count'] > 0 or data['vehicle_count'] > 0 or data['person_count'] > 0
